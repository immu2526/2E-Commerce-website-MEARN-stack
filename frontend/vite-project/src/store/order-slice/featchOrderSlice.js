...
export const featchOrder = createAsyncThunk("featch/order", async (userId) => {
  const { data } = await axios.get(
    `http://localhost:3000/api/shop/order/payment/${userId}/allorder`
  );
  return data;
});
...
export const featchOrderDetails = createAsyncThunk(
  ...
  async (id) => {
    const { data } = await axios.get(
      `http://localhost:3000/api/shop/order/payment/details/${id}`
    );
...
export const allOrder = createAsyncThunk("all/order", async () => {
  const { data } = await axios.get(
    "http://localhost:3000/api/shop/order/payment"
  );
